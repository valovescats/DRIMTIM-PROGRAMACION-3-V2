...
CREATE TABLE IF NOT EXISTS `mydb`.`ComprobanteDePago` (
  ...
  `arregloDatosDelCorrelativo` INT NULL DEFAULT NULL,
...
CREATE UNIQUE INDEX `nombre_UNIQUE` ON `mydb`.`Catalogo` (`nombre` ASC) VISIBLE;

CREATE INDEX `fk_Catalogo_Tienda1_idx` ON `mydb`.`Catalogo` (`Tienda_idTienda` ASC) VISIBLE;
...
CREATE INDEX `fk_LineaDeRopa_Catalogo1_idx` ON `mydb`.`LineaDeRopa` (`Catalogo_idCatalogo` ASC) VISIBLE;
...
CREATE INDEX `fk_Lote_MovimientoAlmacen1_idx` ON `mydb`.`Lote` (`MovimientoAlmacen_idMovAlmacen` ASC, `MovimientoAlmacen_Almacen_idAlmacen` ASC) VISIBLE;
...
-- -----------------------------------------------------
-- procedure OBTENER_FALDA_X_IDFalda
-- -----------------------------------------------------

DELIMITER $$
USE `mydb`$$
CREATE DEFINER=`admin`@`%` PROCEDURE `OBTENER_FALDA_X_IDFalda`(
    IN _id_prenda INT
)
BEGIN
    SELECT  p.idPrenda, p.nombre, p.stockPrenda, p.precioUnidad, p.precioMayor, p.precioDocena,
            p.alertaMinStock, p.color, p.material,
            f.tipo_falda, f.largo, f.con_forro, f.con_volantes
      FROM  Prenda p
      JOIN  Falda  f ON f.idPrenda = p.idPrenda
     WHERE  p.idPrenda = _id_prenda;
END$$
...
-- -----------------------------------------------------
-- procedure OBTENER_POLO_X_IDPolo
-- -----------------------------------------------------

DELIMITER $$
USE `mydb`$$
CREATE DEFINER=`admin`@`%` PROCEDURE `OBTENER_POLO_X_IDPolo`(
    IN _id_prenda INT
)
BEGIN
    SELECT  p.idPrenda, p.nombre, p.stockPrenda, p.precioUnidad, p.precioMayor, p.precioDocena,
